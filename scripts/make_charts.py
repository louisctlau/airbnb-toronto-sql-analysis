...
Usage: python3 scripts/make_charts.py   (run from project root)
"""
import sqlite3
...
DB = ROOT / "data" / "airbnb_toronto.db"
...
ax.set_title("Toronto Airbnb supply — top 15 neighbourhoods\n(Inside Airbnb snapshot, June 2026)")
...
ax.set_title("Median nightly price by room type\n(Inside Airbnb snapshot, June 2026)")
...
fig.suptitle("Superhost vs regular host (June 2026 snapshot)", fontweight="bold", fontsize=13)
...
ax.set_title("Higher ratings command higher prices\n(Toronto Airbnb, June 2026)")
ax.yaxis.set_major_formatter(FuncFormatter(cad))
ax.set_ylim(0, max(vals) * 1.18)
...
ax.set_title("Occupancy is highest in the mid-availability sweet spot\n(Inside Airbnb estimates, June 2026)")
...
ax1.set_title("Each extra bedroom costs more than the last\n(entire homes/apts, Toronto, June 2026)")
...
print("\nDone — 6 charts written to", OUT.relative_to(ROOT))
